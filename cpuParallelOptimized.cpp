...
public:
	...
	void run(vector<Point> & points)
	{
		...
		while(true){
			...
			if(done.load() == true || iter >= max_iterations){
		...
		FILE *file = fopen("valueParallelGoat.csv", "w");
		...
		for(int i = 0; i < K; i++){
			printf("Cluster: %d, Number: %d\n",i,potato[i]);
		}
		fclose(file);

		cout << " Value Parallel Goat TOTAL EXECUTION TIME = "<<std::chrono::duration_cast<std::chrono::microseconds>(end-begin).count()<<"\n";
		cout << "Value Parallel Goat TIME PHASE 1 = "<<std::chrono::duration_cast<std::chrono::microseconds>(end_phase1-begin).count()<<"\n";
        cout << "Value Parallel Goat TIME PHASE 2 = "<<std::chrono::duration_cast<std::chrono::microseconds>(end-end_phase1).count()<<"\n";
	}
};
